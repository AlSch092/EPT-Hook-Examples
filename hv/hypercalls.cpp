...
void set_execute_bit_pfn(vcpu* const cpu) {
    auto const pfn = cpu->ctx->rcx; //uint64_t
    auto const executable = cpu->ctx->rdx; //boolean
    ...
    pte->execute_access = (uint64_t)executable;
