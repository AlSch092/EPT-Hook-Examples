...
void set_execute_bit_pfn(uint64_t const phys_address, const bool executable);
...
// try tracepoint
inline void set_execute_bit_pfn(uint64_t const phys_address, const bool executable) {
    hv::hypercall_input input;
    input.code = hv::hypercall_set_execute_bit_pfn;
    input.key = hv::hypercall_key;
	input.args[0] = phys_address;
    input.args[1] = executable;
    hv::vmx_vmcall(input);
}
